...
namespace LitJson
{
	public class JsonWriter
	{
		...
		public JsonWriter(TextWriter writer)
		{
			if (writer == null)
			{
				JsonException.Throw(new ArgumentNullException("writer"));
			}
			this.writer = writer;
			this.Init();
		}
		...
		private void DoValidation(Condition cond)
		{
			...
			if (this.validate)
			{
				...
				{
					JsonException.Throw(new JsonException("A complete JSON symbol has already been written"));
				}
				switch (cond)
				{
				case Condition.InArray:
					...
					{
						JsonException.Throw(new JsonException("Can't close an array here"));
					}
					break;
				case Condition.InObject:
					if (!this.context.InObject || this.context.ExpectingValue)
					{
						JsonException.Throw(new JsonException("Can't close an object here"));
					}
					break;
				case Condition.NotAProperty:
					if (this.context.InObject && !this.context.ExpectingValue)
					{
						JsonException.Throw(new JsonException("Expected a property"));
					}
					break;
				case Condition.Property:
					if (!this.context.InObject || this.context.ExpectingValue)
					{
						JsonException.Throw(new JsonException("Can't add a property here"));
					}
					break;
				case Condition.Value:
					if (!this.context.InArray && (!this.context.InObject || !this.context.ExpectingValue))
					{
                        JsonException.Throw(new JsonException("Can't add a value here"));
					}
					break;
				}
			}
		...
		public override string ToString()
		{
			...
			{
				result = this.inst_string_builder.ToString();
			}
			return result;
		}
