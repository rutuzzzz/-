...
namespace LitJson
{
	public class JsonReader
	{
		...
		private JsonReader(TextReader reader, bool owned)
		{
			if (reader == null)
			{
				JsonException.Throw(new ArgumentNullException("reader"));
			}
		...
		private void ProcessNumber(string number)
		{
			...
			else if (long.TryParse(number, out num3))
			{
				...
				while (!this.parser_return)
				{
					...
					if (this.current_symbol == this.current_input)
					{
						if (!this.ReadToken())
						{
							...
							{
								JsonException.Throw(new JsonException("Input doesn't evaluate to proper JSON text"));
							}
							result = this.parser_return;
							return result;
						}
					}
					else
					{
						...
						{
							JsonException.Throw(new JsonException((ParserToken)this.current_input, inner_exception));
							return false;
						}
						if (array[0] != 65554)
						{
